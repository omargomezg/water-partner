...
public class ReportService {
	...
	public MunicipalReportDto getMunicipalReportByPeriod(Long periodId, int pageIndex, int pageSize)
			throws PeriodException {
		PeriodEntity period = periodRepository.findById(periodId).orElseThrow(PeriodException::new);

		// Optional<PeriodEntity> lastPeriod =
		// periodRepository.findFirstByIdNot(periodId);
		...
		List<ReportDto> detail = page.getContent().stream()
				.map(consumption -> ReportDto.builder().consumption(consumption.getActualRecord().intValue())
						.codeCommune(14101)
						.beneficiary(BeneficiaryDto.builder().dni(consumption.getDni())
								.fullName(consumption.getFullName()).address("Curiñanco").build())
						.nameCommune("VALDIVIA").decree(new DecreeDto(132423L, new Date())).build())
