...
public class FlowServiceImpl implements FlowService {
	...
	public PaymentOrderResponse sendPaymentOrder(PendingPaymentDto paymentOrder)
		...
		String result = restTemplate.postForObject(String.format("%s/api/payment/create?token=%s}", flowUrl, flowToken),
	...
	public PaymentEntity findByToken(String token) {
		return paymentRepository.findByToken(token).orElseThrow(() -> new PaymentNotFoundException(token));
	}
	...
	@Override
	public PaymentOrderStatusResponse findFlowPaymentStatusByToken(String token)
		...
		String signature = getSignature(body);
		String url = String.format("%s/api/payment/getStatus?apiKey=%s&token=%s&s=%s", flowUrl, flowApiKey, token,
				signature);
		var restTemplate = new RestTemplate();
	 ...
	 */
	private String getSignature(MultiValueMap<String, String> body)
			throws NoSuchAlgorithmException, InvalidKeyException {
		var stringBuilder = new StringBuilder();
		List<String> keyList = new ArrayList<>(body.keySet());
		Collections.sort(keyList);
		for (String key : keyList) {
			stringBuilder.append(key).append(body.get(key).get(0));
		}
